import {  Autocomplete, Box, Button, Card, CardActions, CardContent, CardHeader, Chip, Grid, IconButton, Link, Paper, Stack, TextField, Typography, styled, Select, MenuItem, InputAdornment, Drawer, Divider } from '@mui/material'
...
const Home = () => {
  ...
  const {tableQueryResult: {data, isLoading, isError, refetch}, 
    ...
    resource: "soumission/offres/lots/all",
  })
  
  let offres = data?.data ?? []
  ...
  const currentFilterValues = useMemo(() => {
    ...
    return {
      nom:
        logicalFilters.find((item : any) => item.field === "nom")?.value || "",
      domaine:
        logicalFilters.find((item : any) => item.field === "domaine")?.value || "",
      ville:
        logicalFilters.find((item : any) => item.field === "ville")?.value || "",
    };
}, [filters]);  
  ...
  const handleDelete = (chipToDelete: any) => () => {
    setChipData((chips : any) => chips.filter((chip : any) => chip !== chipToDelete));
  };
  ...
  const handleAddFav = (id: number) => {
    axiosInstance.post("/soumission/favoris", {offreId: id}).then(() => {
      offres = offres.filter((offre :any) => offre.id !== id);
      refetch()
    })
  }
  ...
  return (
    ...
    <Box sx={{ width: '100%'  }}>
        <Grid container spacing={2} >
          ...
          <Card sx={{bgcolor: "success.main", padding: '10px' , borderRadius: 20}} >
                                      ...
                                      <CardActions disableSpacing>
                                        {/*<Button  onClick={() => handleShowDialog(offre.id)} sx={{ml:'10px', color: 'white',borderRadius: "10px", backgroundColor: "primary.main",":hover": {backgroundColor: "success.main", color: "primary.main"} }}>Soumissionner</Button>*/}
                                          ...
                                          <IconButton aria-label="add to favorites">
                                            {offre.favorite ?  <FavoriteIcon onClick={() => {handleRemoveFav(offre.id)}}  sx={{color: "primary.main"}}  /> : <FavoriteBorderIcon onClick={() => {handleAddFav(offre.id)}} sx={{color: "primary.main"}}  /> }
                                          </IconButton>
                                      </CardActions>
                                      <Divider/>
                                      <Box sx={{padding: 2}}>                                      
