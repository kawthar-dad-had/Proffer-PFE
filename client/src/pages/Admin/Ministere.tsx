...
export const Ministere = () => {
  ...
  const { dataGridProps } = useDataGrid({resource: `auth/users/ministeres/`,});
  ...
  const fetchData = async () => {
      await axiosInstance.get(`auth/users/ministeres/?filters=${JSON.stringify(
        state.filter
  ...
  const handleCreate = async() => {
    ...
    await axiosInstance.post('/auth/users/ministere', formData).then((res) => {
      if (res.status === 201) {
        Swal.fire("Succés", "Ministère créé avec succés", "success");
        setMinObj({
          nom: "",
          email: "",
          password: "",
              ...
              <Grid container mb={"20px"}>
                      <Grid container item xs={6} >
                            ...
                            <div role="presentation" >
                              <Breadcrumbs aria-label="breadcrumb">
                                ...
                                <StyledBreadcrumb
                                  component="a" 
                                  href="#" 
                                  onClick={() => { navigate("/ministere/create")}}
                                  label="Ajouter un ministère"
                          ...
                          <Grid item xs={10}  mt={1}>
                            <Typography variant="h4">Ministères</Typography>
                          </Grid>
                      ...
                      <Grid container height={3} item xs={6} justifyContent={'right'} mt={"20px"}>
                        <Button  onClick={() =>  setOpen(true)}sx={{ml:'10px', color: 'white',borderRadius: "10px", backgroundColor: "primary.main",":hover": {backgroundColor: "#fff", color: "primary.main"} }}>Ajouter un ministère</Button>
                      </Grid>
              </Grid>
              <DataGrid
              {...dataGridProps}
      ...
      <Dialog open={open} fullWidth={true} maxWidth={"lg"} >
          <DialogTitle sx={{color: "primary.main", textAlign: "center"}} >
              <Grid container justifyContent={"center"}>
                    <BusinessCenterIcon fontSize='large'/>
                    <Typography variant="h4">Créer un ministère</Typography>
              </Grid>
              </DialogTitle>
          <DialogContent>
            <DialogContentText>
